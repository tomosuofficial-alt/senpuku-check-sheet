...
// ============================================================
// [診断用] トイレ清掃の未実施判定を任意の営業日で検証
// ============================================================

/** 指定営業日のトイレ清掃ログと未実施判定をログ出力 */
function verifyToiletMissed_(testBd) {
  var storeId = 'STORE001';
  var ss = SpreadsheetApp.openById(SHEET_ID);
  var histSheet = ss.getSheetByName(SHEETS.HISTORY);
  var histData = (histSheet && histSheet.getLastRow() > 1) ? histSheet.getDataRange().getValues() : [];
  ...
  for (var i = 1; i < histData.length; i++) {
    ...
    var hbd = businessDateFromTimestamp_(h[0]);
    var nr = normalizeHistoryRow_(h);
    if (hbd !== testBd || nr.storeId !== storeId || nr.category !== 'トイレ清掃') continue;
    var ts = h[0];
    var d = (ts instanceof Date) ? ts : new Date(String(ts));
    var hh = parseInt(Utilities.formatDate(d, 'Asia/Tokyo', 'H'), 10);
    var hhmm = Utilities.formatDate(d, 'Asia/Tokyo', 'HH:mm');
    doneHours[hh] = true;
    rows.push(hhmm + ' (hour=' + hh + ')');
  }
  ...
  Logger.log('=== ' + testBd + ' のトイレ清掃検証 ===');
  Logger.log('実施ログ件数: ' + rows.length);
  Logger.log('実施時刻: ' + (rows.length ? rows.join(', ') : '(なし)'));
  Logger.log('doneHours: ' + JSON.stringify(doneHours));
  Logger.log('--- 未実施判定 ---');
  Logger.log(missed.length > 0 ? missed.join(', ') : '(全スロット実施済み)');
}

/** 2026-05-11 営業日のトイレ清掃を検証 */
function verify_2026_05_11() {
  verifyToiletMissed_('2026-05-11');
}
