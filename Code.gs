...
// ============================================================
// [テスト用] 過去 NG だった写真を新プロンプトで再判定
// スプレッドシート保存・LINE通知はスキップ。Logger.log のみ。
// 検証完了後は削除して問題なし。
// ============================================================

function testPastNGPhotos() {
  var testCases = [
    {
      fileId: '1WRokO4oocTkbDCBbK_P8RBW6mt8jUK5o',
      itemName: 'コンロの清掃',
      category: '閉店',
      memo: '五徳の経年焦げ — 旧プロンプトでNG x6'
    },
    {
      fileId: '1DvrhPPpnmrQHX6DLvMspQzVQkshDi5ZI',
      itemName: 'おしぼりの補充',
      category: '開店',
      memo: 'おしぼりウォーマー満杯 — 旧プロンプトで「蒸し器」誤認NG'
    },
    {
      fileId: '1xkS3PGzTnzJjygG8wh9gDJDKeVUbRisR',
      itemName: '予約確認・座席セット',
      category: '開店',
      memo: '完璧にセット済み — 旧プロンプトで「懐石風景・対象外」誤認NG'
    }
  ];

  for (var i = 0; i < testCases.length; i++) {
    var t = testCases[i];
    Logger.log('=== Test ' + (i + 1) + '/' + testCases.length + ': ' + t.itemName + ' ===');
    Logger.log('memo: ' + t.memo);
    Logger.log('fileId: ' + t.fileId);
    try {
      var blob = DriveApp.getFileById(t.fileId).getBlob();
      var base64 = Utilities.base64Encode(blob.getBytes());
      var mimeType = blob.getContentType() || 'image/jpeg';
      var dataUrl = 'data:' + mimeType + ';base64,' + base64;

      var result = verifyPhotoWithAI_(dataUrl, t.itemName, t.category);

      Logger.log('result: ' + result.result + (result.result === 'OK' ? ' ✓' : ' ✗'));
